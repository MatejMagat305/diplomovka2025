...
SYCL_EXTERNAL void writeMinimalPath(const MemoryPointers& globalMemory) {
	...
	if (minSize < 0){
		globalMemory.minSize_numberColision_error[MINSIZE] = 0;
	}
	else {
		globalMemory.minSize_numberColision_error[MINSIZE] = minSize;
	}
}
...
void pushVector(const std::vector<std::vector<Position>>& vec2D, Map* m) {
	...
	CPUMemory.minSize_numberColision_error[MINSIZE] = minSize;
}
