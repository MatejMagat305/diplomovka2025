...
SYCL_EXTERNAL bool isBlocked(int agentId, int step, Position next, MemoryPointers& localMemory) {
    int numConstraints = localMemory.numberConstraints[agentId];
    for (int i = 0; i < numConstraints; i++) {
        Constraint c = localMemory.constraints[i];
        if (c.position.x == next.x && c.position.y == next.y && c.timeStep == step) {
            return true;
        }
    }
    return false;
    ...
    while (!empty(myHeap)) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            if (next.x >= 0 && next.x < width && next.y >= 0 && next.y < height &&
                ...
                if (isBlocked(agentId, newG, next, localMemory)) {
                    continue;
                }
...
std::vector<Position> ComputeASTAR(Map* m, int agentID, const std::vector<std::vector<Constraint>>& constraints) {
    ...
    std::unordered_set<Position> constraintSet;
    if (!constraints.empty()) {
        for (const auto& constraint : constraints[agentID]) {
            constraintSet.insert(constraint.position);
        }
    }
    ...
    while (!openSet.empty()) {
        ...
        for (auto& next : neighbors) {
            ...
            if (constraintSet.find(next) != constraintSet.end()) continue; 
