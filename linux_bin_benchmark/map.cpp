...
void Map::deleteMemory() {
    ...
    delete[] CPUMemory.minSize_maxtimeStep_error; 
    CPUMemory.minSize_maxtimeStep_error = nullptr;
    delete[] CPUMemory.agentPaths; 
...
void Map::initAgents() {
    ...
    for (int i = 0; i < CPUMemory.agentsCount; i++) {
        ...
        frame.x = (double)agent.position.x;
        frame.y = (double)agent.position.y;
        agent.loaderCurrent = rand() % CPUMemory.loadersCount;
        agent.unloaderCurrent = rand() % CPUMemory.unloadersCount;
...
void Map::initMem() {
    ...
    try {
        CPUMemory.minSize_maxtimeStep_error = new int[3];
		...
		CPUMemory.minSize_maxtimeStep_error[0] = -200;
        CPUMemory.minSize_maxtimeStep_error[1] = 0;
        CPUMemory.minSize_maxtimeStep_error[2] = 0;
...
void Map::AddAgent(int x, int y) {
	...
	a.sizePath = 0;
	a.goal = CPUMemory.loaderPositions[a.loaderCurrent];
    CPUMemory.agents[CPUMemory.agentsCount] = a;
	CPUMemory.agentsCount++;
}
...
void Map::print(){
	...
	for (int y = 0; y < CPUMemory.height; y++) {
		for (int x = 0; x < CPUMemory.width; x++) {
            if (isAgentIn(x, y, CPUMemory.agentsCount)){
                std::cout << "a";
				continue;
            }
			std::cout << CPUMemory.grid[getTrueIndexGrid(CPUMemory.width, x, y)];
...
Map* Map::copy()  {
    ...
    // Skopíruj jednoduché èleny
    newMap->obstacleCount = this->obstacleCount;
    newMap->selected = this->selected;
    newMap->capacityAgents = this->capacityAgents;
    newMap->capacityLoaders = this->capacityLoaders;
    newMap->capacityUnloaders = this->capacityUnloaders;

    // Alokuj a skopíruj agentFrames
    if (this->agentFrames) {
        newMap->agentFrames = new AgentFrame[this->CPUMemory.agentsCount];
        std::copy(this->agentFrames, this->agentFrames + this->CPUMemory.agentsCount, newMap->agentFrames);
    }

    // Skopíruj MemoryPointers
    MemoryPointers& src = this->CPUMemory;
    MemoryPointers& dst = newMap->CPUMemory;

    dst.width = src.width;
    dst.height = src.height;
    dst.loadersCount = src.loadersCount;
    dst.unloadersCount = src.unloadersCount;
    dst.agentsCount = src.agentsCount;

    int cells = dst.width * dst.height;
	int stackSize = dst.agentsCount * cells;
    int agentsCount = dst.agentsCount;
    int loaderCount = dst.loadersCount;
    int unloaderCount = dst.unloadersCount;

#define COPY_ARRAY(TYPE, FIELD, COUNT) \
        if (src.FIELD) { \
            dst.FIELD = new TYPE[COUNT]; \
            std::copy(src.FIELD, src.FIELD + COUNT, dst.FIELD); \
        }

    COPY_ARRAY(int, gCost, stackSize);
    COPY_ARRAY(int, fCost, stackSize);
    COPY_ARRAY(int, minSize_maxtimeStep_error, 3);
    COPY_ARRAY(char, grid, cells);
    COPY_ARRAY(Position, loaderPositions, loaderCount);
    COPY_ARRAY(Position, unloaderPositions, unloaderCount);
    COPY_ARRAY(Position, agentPaths, stackSize); 
    COPY_ARRAY(Position, cameFrom, stackSize);
    COPY_ARRAY(Agent, agents, agentsCount);
    COPY_ARRAY(bool, visited, stackSize);
    COPY_ARRAY(HeapPositionNode, openList, stackSize);
