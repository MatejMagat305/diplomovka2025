...
std::string initializeSYCLMemory(Map* m) {
	...
	try {
		...
		GPUMemory.minSize_maxtimeStep_error = sycl::malloc_device<int>(3, q);
...
void synchronizeCPUFromGPU(Map* m) {
	...
	sycl::event e4 = q.memcpy(CPUMemory.minSize_maxtimeStep_error, GPUMemory.minSize_maxtimeStep_error, sizeof(int)*3);
...
void synchronizeGPUFromCPU(Map* m) {
	...
	sycl::event e4 = q.memcpy(GPUMemory.minSize_maxtimeStep_error, CPUMemory.minSize_maxtimeStep_error, sizeof(int)*3);
	sycl::event::wait_and_throw({ e1, e3, e4 });
}
...
void deleteGPUMem() {
	sycl::queue& q = getQueue();
	q.wait();
	freePtr((void**)(&(GPUMemory.minSize_maxtimeStep_error)));
...
void SYCL_AStar(Map* m) {
	...
	try {
		...
		m->CPUMemory.minSize_maxtimeStep_error[ERROR] = -1;
	}
	catch (const std::exception& e) {
		m->CPUMemory.minSize_maxtimeStep_error[ERROR] = -2;

	}
	catch (...) {
		m->CPUMemory.minSize_maxtimeStep_error[ERROR] = -3;
	}
}
	...
	try {
		...
		auto foundPathsAgent = q.submit([&](sycl::handler& h) {
			...
			h.parallel_for(sycl::range<1>(std::min(workSize, GPUMemory.agentsCount)), [=](sycl::id<1> idx) {
				...
				MemoryPointers localMemory;
				fillLocalMemory(globalMemory, id, localMemory);
				if (astar_algorithm_get_sussces(id, localMemory)) {
					localMemory.agents[id].sizePath = reconstructPath(id, localMemory);
				}
				else {
					globalMemory.minSize_maxtimeStep_error[ERROR] = id + 1;
				}
				});
			});
		foundPathsAgent.wait_and_throw();
		// kolízie
		...
		std::vector<std::vector<Position>> pathsFromGPU = getVector(m);
		CBSNode solution = solveCBS(pathsFromGPU);
		auto end_time = std::chrono::high_resolution_clock::now();
		pushVector(solution.paths, m);
		auto copy_time = std::chrono::high_resolution_clock::now();
		result.timeRun = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
		result.timeSynchronize = std::chrono::duration_cast<std::chrono::microseconds>(copy_time - end_time).count();
	}
	catch (const sycl::exception& e) {
		result.timeRun = 0;
		result.timeSynchronize = 0;
		result.error = "SYCL exception caught: ";
	}
	catch (const std::exception& e) {
