...
Scene* ParameterScene::DrawControl(){
    ...
    if (GuiValueBox({ 300, 180, 200, 20 }, "Agents", &agentCount, 1, 64, activeAgents)) {
        activeAgents = !activeAgents;
        alert = activeWidth = activeHeight = activeObstacles = activeMarkers = activeLoaders = activeUnloaders = false;
    }
    if (GuiValueBox({ 300, 220, 200, 20 }, "Obstacles", &obstacleCount, 1, 8, activeObstacles)) {
    ...
    if (GuiValueBox({ 300, 300, 200, 20 }, "Loaders", &loaderCount, 1, 24, activeLoaders)) {
    ...
    if (GuiValueBox({ 300, 340, 200, 20 }, "Unloaders", &unloaderCount, 1, 24, activeUnloaders)) {
