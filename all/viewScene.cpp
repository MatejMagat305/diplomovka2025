...
Scene* ViewScene::DrawControl() {
    ...
    if (GuiButton(Rectangle{ 380, static_cast<float>(GetWindowHeight()) - 40, 100, 30 }, "Start without save")) {
        return StartSimulation();
    }

    if (GuiButton(Rectangle{ 490, static_cast<float>(GetWindowHeight()) - 40, 100, 30 }, "Change")) {
