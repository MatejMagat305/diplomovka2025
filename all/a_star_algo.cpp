	...
	while (!empty(myHeap)) {
		...
		for (int j = 0; j < 4; j++) {
			...
			if (next.x >= 0 && next.x < width && next.y >= 0 && next.y < height &&
				localMemory.grid[getTrueIndexGrid(width, next.x, next.y)] != OBSTACLE_SYMBOL && 
				!localMemory.visited[getTrueIndexGrid(width, next.x, next.y)]) {

				int newG = localMemory.gCost[getTrueIndexGrid(width, current.x, current.y)] + 1;
				if (newG < localMemory.gCost[getTrueIndexGrid(width, next.x, next.y)]) {
					localMemory.gCost[getTrueIndexGrid(width, next.x, next.y)] = newG;
					localMemory.fCost[getTrueIndexGrid(width, next.x, next.y)] =
						newG + myAbs(goal.x - next.x) + myAbs(goal.y - next.y);
					localMemory.cameFrom[getTrueIndexGrid(width, next.x, next.y)] = current;
