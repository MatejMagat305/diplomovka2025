...
Scene* InfoScene::DrawControl(){
	...
	if (m.CPUMemory.minSize_maxtimeStep_error[ERROR] != 0){
		DrawText("The simulation is not able to solve", 50, GetWindowHeight() / 2, 20, RED);
		DrawText(", press SPACE to get back to main menu", 50, GetWindowHeight() / 2+50, 20, RED);
		DrawText(mem->info.error.c_str(), 50, GetWindowHeight() / 2 + 70, 10, RED);
		...
		std::stringstream ss;
		ss << "total time: " << mem->info.timeRun + mem->info.timeSynchronize << " ms";
		DrawText(ss.str().c_str(), 50, GetWindowHeight() / 2 + 30, 20, RED);
		ss = std::stringstream();
		ss << "compute time: " << mem->info.timeRun << " ms";
		DrawText(ss.str().c_str(), 50, GetWindowHeight() / 2 + 50, 20, RED);
		ss = std::stringstream();
		ss << "synchronize time: " << mem->info.timeSynchronize << " ms";
